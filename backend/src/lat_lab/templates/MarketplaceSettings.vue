...
export default {
  ...
  methods: {
    ...
    async fetchSettings() {
      ...
      try {
        const response = await fetch('/api/plugins/marketplace/source');
        if (!response.ok) {
    ...
    async fetchPlugins() {
      ...
      try {
        const response = await fetch('/api/plugins/marketplace/plugins');
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
    ...
    async refreshPlugins() {
      ...
      try {
        const response = await fetch('/api/plugins/marketplace/refresh', {
          method: 'POST'
        });
