#!/usr/bin/env python3
"""
LAT-LAB SQLite数据库初始化脚本
用于传统安装方式，专门处理SQLite数据库
"""
import os
import sys
import logging
...
def create_sqlite_tables():
    ...
    try:
        ...
        logger.error(f"创建SQLite数据库表失败: {e}")
        import traceback
...
def main():
    """主函数"""
    logger.info("开始SQLite数据库初始化...")
    
    # 创建表
    if not create_sqlite_tables():
        logger.error("创建SQLite数据库表失败")
        sys.exit(1)
